...
# Notify
notify_user() {
	notify-send -h int:value:$(get_volume) -h "string:x-dunst-stack-tag:volume_notif" -u low "Volume : $(get_volume) %"
}
...
# Notify
notify_mic_user() {
	notify-send -h int:value:$(pamixer --default-source --get-volume) -h "string:x-dunst-stack-tag:volume_notif" -u low "Mic-Level : $(pamixer --default-source --get-volume) %"
}
